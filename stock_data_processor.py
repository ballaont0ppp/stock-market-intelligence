...
from data_validation import validate_stock_data, clean_stock_data, print_dataframe_info
    ...
    def get_historical_data(self, symbol, period_years=2):
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    data = yf.download(symbol, start=start, end=end, timeout=30)
                    
                    # Handle different yfinance return types
                    if data is None:
                    ...
                    # Convert to DataFrame if needed (yfinance returns DataFrame)
                    if isinstance(data, pd.DataFrame):
                        df = data.copy()
                    else:
                        # Fallback if somehow not a DataFrame
                        df = pd.DataFrame(data)
                    
                    # Check if we actually have data (different from empty)
                    if len(df) == 0:
                        if attempt == max_retries - 1:
                            self.logger.error(f"No data returned for symbol {symbol} after {max_retries} attempts")
                            return None
...
if __name__ == "__main__":
    # Create an instance of StockDataProcessor with debug enabled
    ...
    if connection_test['connection_ok']:
        # Example: Get historical data for Apple (AAPL)
        symbol = "AAPL"
        print(f"\nGetting historical data for {symbol}...")
        data = processor.get_historical_data(symbol)
        
        if data is not None:
            # Preprocess the data
            print(f"Preprocessing data for {symbol}...")
            processed_data = processor.preprocess_data(data, symbol)
            
            if processed_data is not None:
                # Get today's data
                print(f"Getting today's data for {symbol}...")
                today_data = processor.get_today_data(processed_data)
                
                if today_data is not None:
                    print(f"Today's stock data for {symbol}:")
                    print(today_data)
                else:
                    print("Failed to get today's data")
            else:
        ...
        invalid_data = processor.get_historical_data("INVALID_SYMBOL_123")
        if invalid_data is None:
            print("Correctly rejected invalid symbol")
    else:
