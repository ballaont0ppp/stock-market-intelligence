    ...
    def predict(self, df, sequence_length=7, epochs=5):
        ...
        try:
            ...
            if 'Close' in df.columns:
                ...
                # Fallback to 4th column for Close price
                if df.shape[1] > 4:
                    close_col_idx = 4
                else:
                    raise ValueError(f"DataFrame has only {df.shape[1]} columns, need at least 5")
            
            # Extract Close price data
            training_set = df.iloc[:, close_col_idx:close_col_idx+1].values
            ...
            try:
                ...
                plt.plot(real_stock_price, label='Actual Price', color='blue')
                plt.plot(predicted_stock_price, label='Predicted Price', color='red', linestyle='--')
                plt.legend(loc=4)
                plt.title('LSTM Model Performance')
                plt.xlabel('Time')
                plt.ylabel('Stock Price')
                plt.grid(True, alpha=0.3)
                plt.savefig('static/LSTM.png', dpi=65, bbox_inches='tight')
                plt.close()
                self.logger.info("LSTM plot saved to static/LSTM.png")
            except Exception as e:
                self.logger.warning(f"Could not save LSTM plot: {e}")
                plt.close()
...
if __name__ == "__main__":
    print("LSTM Model ready for testing")